...
var noun = ["linguinis", "chicken sandwich(s)", "lasagna", "restaurant", "outfit(s)"];
...
var sentences = ["I didn't become a overnight success.", sentence];

    getQuote();
    function getQuote(){
        if (counter<100){
    sentence = subject[Math.floor(Math.random()*subject.length)] + " " + noun[Math.floor(Math.random()*noun.length)] + " is " + adjective[Math.floor(Math.random()*adjective.length)] + ".";
    sentences = ["I didn't become an overnight success.", sentence];
    document.getElementById("quote").innerHTML = sentences[Math.floor(Math.random()*sentences.length)];
    counter +=1;
        }
        else{
            document.getElementById("quote").innerHTML = "Get out of my restaurant rude customer!";
        }
    }
